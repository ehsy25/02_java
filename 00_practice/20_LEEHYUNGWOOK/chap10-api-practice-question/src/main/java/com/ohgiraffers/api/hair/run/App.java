...
public class App {
    public static void main(String[] args) {
        ...
        String[] firstTokenization = data.split("\n");
        User[] users = new User[firstTokenization.length-1]; //첫번째 줄 제외 100
        for ( int i = 1; i < firstTokenization.length; i++ ) {
            String[] secondTokization = firstTokenization[i].split(",");
            int id =  Integer.parseInt(secondTokization[0]);
            String firstName =  secondTokization[1];
            String last_name =  secondTokization[2];
            String email =  secondTokization[3];
            String gender =  secondTokization[4];
            String birthday =  secondTokization[5];
            String reservation_date =  secondTokization[6];
            String reservation_time =  secondTokization[7];
            String created_at =  secondTokization[8];
            if (birthday.equals("")) {
                b_day = null;
            } else { b_day =  LocalDate.parse(birthday, DateTimeFormatter.ofPattern("yyyy/MM/dd")); }
            if (reservation_date.equals("")) {
                r_day = null;
            } else {r_day = LocalDate.parse(reservation_date);}
            if (reservation_time.equals("")) {
                r_time = null;
            } else {r_time =  LocalTime.parse(reservation_time, DateTimeFormatter.ofPattern("H:mm"));}
            if (created_at.equals("")) {
                c_date = null;
            } else {c_date = LocalDateTime.parse(created_at);}

            users[i-1] = new User(id,firstName,last_name,email,gender,b_day,r_day,r_time,c_date);
            //id,first_name,last_name,email,gender,birthday,reservation_date,reservation_time,created_at
        }
        for ( User u : users ) {
