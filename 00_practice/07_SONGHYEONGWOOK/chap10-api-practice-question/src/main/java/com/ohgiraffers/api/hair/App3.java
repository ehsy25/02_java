...
public class App3 {
    public static void main(String[] args) {
        ...
        System.out.println("첫 번째 데이터 라인: " + infos[1]);
        System.out.println("--- 파싱 시작 ---");


        // 1. 루프 조건 수정: i < infos.length
        for (int i = 1; i < infos.length; i++) {
            String line = infos[i].trim();

            // 2. 공백이 제거된 'line'을 토큰화합니다.
            ...
            try {
                ...
                // 4. 빈 생년월일 처리
                String birthdayStr = st.nextToken();
                LocalDate birthday = null; // 기본값 null
                if (!birthdayStr.isEmpty()) {
                    birthday = LocalDate.parse(birthdayStr, DateTimeFormatter.ofPattern("yyyy/MM/dd"));
                }

                LocalDate reservationDate = LocalDate.parse(st.nextToken(), DateTimeFormatter.ofPattern("yyyy-MM-dd"));

                // 5. 한 자리 시간 처리 (H:mm)
                String timeStr = st.nextToken();
                LocalTime reservationTime = LocalTime.parse(timeStr, DateTimeFormatter.ofPattern("H:mm"));

                LocalDateTime createdTime = LocalDateTime.parse(st.nextToken(), DateTimeFormatter.ofPattern("yyyy-MM-ddTHH:mm:ss"));

                users[index] = new User(id, firstName, lastName, email, gender, birthday, reservationDate, reservationTime, createdTime);
                index++;

            } catch (DateTimeParseException e) {
                System.err.println(i + "번째 줄 날짜/시간 파싱 오류: " + e.getMessage());
            } catch (NoSuchElementException e) {
                System.err.println(i + "번째 줄 토큰 오류 (데이터 부족): " + e.getMessage());
            } catch (NumberFormatException e) {
                System.err.println(i + "번째 줄 ID 파싱 오류: " + e.getMessage());
            }
        }

        System.out.println("--- 파싱 완료 (총 " + index + "건) ---");


        // 6. 출력 루프 시작 인덱스 수정: i = 0
        for (int i = 0; i < index; i++) {
            System.out.println(users[i].information());
        }
    }
}
