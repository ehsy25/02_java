package com.rlatjddms.api.hair.run;

import com.rlatjddms.api.hair.model.dto.User;
...
public class App {
    ...
    public static void main(String[] args) {
        ...
        1,Devin,Rossey,drossey0@state.tx.us,Male,1983/04/04,2024-09-04,20:22,2012-10-10T07:55:50
        ...
        for(int i = 1; i < arr.length; i++) {
            ...
            int id = Integer.parseInt(members[0]);
            String firstname = members[1];;
            String lastname = members[2];;
            String email = members[3];
            String gender = members[4];
            LocalDate birthday = null;
            LocalDate reservationDate = null;
            LocalTime reservationTime = null;
            LocalDateTime createdAt = null;

            if(!members[5].isEmpty()) birthday = LocalDate.parse(members[5], DateTimeFormatter.ofPattern("yyyy/MM/dd"));
            if(!members[6].isEmpty()) reservationDate = LocalDate.parse(members[6]);
            if(!members[7].isEmpty()) reservationTime = LocalTime.parse(members[7], DateTimeFormatter.ofPattern("H:mm"));
            if(!members[8].isEmpty()) createdAt = LocalDateTime.parse(members[8]);

            User user = new User(id, firstname, lastname, email, gender, birthday, reservationDate, reservationTime, createdAt);
