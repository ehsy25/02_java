...
public class App {
    public static void main(String[] args) {
        ...
        User[] user = new User[rows.length];
        ...
        for (int i = 0; i < rows.length; i++) {
            ...
            String email = f[3].trim();
            String gender = f[4].trim();
            String birthStr = f[5].trim();
            LocalDate birth = null;
            if (!birthStr.isEmpty()) {
                if (birthStr.contains("/")) {
                    birth = LocalDate.parse(birthStr, DateTimeFormatter.ofPattern("yyyy/MM/dd"));
                } else {
                    birth = LocalDate.parse(birthStr, DateTimeFormatter.ofPattern("yyyy-MM-dd"));
                }
            }
            String rdateStr = f[6].trim();
            LocalDate reservationDate = rdateStr.isEmpty()
                    ? null
                    : LocalDate.parse(rdateStr, DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            String rtimeStr = f[7].trim();
            LocalTime resprervationTime = rtimeStr.isEmpty()
                    ? null
                    : LocalTime.parse(rtimeStr, DateTimeFormatter.ofPattern("H:mm"));
            String rendStr = f[8].trim();
            LocalDateTime reservationEndDate = rendStr.isEmpty()
                    ? null
                    : LocalDateTime.parse(rendStr, DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss"));

            user[i] = new User(id, firstName, lastName, email, gender,
                    birth, reservationDate, resprervationTime, reservationEndDate);
        }
