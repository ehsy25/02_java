    ...
    public static void main(String[] args) {
        ...
        for(int i=0; i<token.length; i++){
           for(int j=0; j<token[i].split(",").length; j++){
               if(token[i].split(",")[j].equals("")){
                   token[i]=token[i].replaceFirst(",,", ",null,");
               }
           }
        }
        ...
        for(int i=0; i<token.length; i++){
            ...
            if (str.equals("null")) {
                ...
                birthday = LocalDate.parse(str.replace("/", "-"));
            }

            reservationDate=LocalDate.parse(t[6]);
            reservationTime = LocalTime.of(Integer.parseInt(t[7].split(":")[0]), Integer.parseInt(t[7].split(":")[1]));
            createdAt=LocalDateTime.parse(t[8]);
            u[cnt++]=new User(id,firstName,lastName,email,gender,birthday,reservationDate,reservationTime,createdAt);
        }

        for(int i=0; i<u.length; i++){
            System.out.println("User{id="+u[i].getId()+", "+"firstName="+"'"+u[i].getFirstName()+"', email= '"+u[i].getEmail()+"', gender="+u[i].getGender()+"birthday="+u[i].getBirthday()+", reservationDate="+u[i].getReservationDate()+", reservationTime="+u[i].getReservationTime()+", createdAt="+u[i].getCreatedAt()+"}");
        }
