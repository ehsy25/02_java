...
public class App {
    public static void main(String[] args) {
        ...
        String[] uArr = data.split("\n");

        for(int i = 1; i < uArr.length; i++){
            String[] u = uArr[i].split(",");
            ...
            if(!u[5].isEmpty()){
                birthday = LocalDate.parse(u[5],DateTimeFormatter.ofPattern("yyyy/MM/dd"));
            }
            if(!u[6].isEmpty()){
                reservation_date = LocalDate.parse(u[6]);
            }
            if(!u[7].isEmpty()){
                reservation_time = LocalTime.parse(u[7], DateTimeFormatter.ofPattern("H:mm"));
            }
            if(!u[8].isEmpty()){
                created_at = LocalDateTime.parse(u[8]);
            }
