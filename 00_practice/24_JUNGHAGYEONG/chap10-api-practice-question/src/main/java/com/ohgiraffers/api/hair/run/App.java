...
public class App {
    public static void main(String[] args) {
        ...
        User[] users = new User[line.length - 1];
        ...
        for (int i = 1; i < line.length; i++) {
            ...
            if (!info[5].isEmpty()) {
                try {
                    birthday = LocalDate.parse(info[5]);
                } catch (Exception e) {
                    DateTimeFormatter format = DateTimeFormatter.ofPattern("yyyy/MM/dd");
                    birthday = LocalDate.parse(info[5], format);
                }
            } else {
                birthday = null;
            }

            LocalDate reservation_date = LocalDate.parse(info[6]);
            DateTimeFormatter format = DateTimeFormatter.ofPattern("H:mm");
            LocalTime reservation_time = LocalTime.parse(info[7], format);
            LocalDateTime created_at = LocalDateTime.parse(info[8]);

            users[index++] = new User(id, first_name, last_name, email, gender, birthday, reservation_date, reservation_time, created_at);
        }
