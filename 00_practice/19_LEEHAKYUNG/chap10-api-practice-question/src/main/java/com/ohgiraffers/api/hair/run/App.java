...
public class App {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < memNum; i++) {
            ...
            LocalDate birthday = (tokens[5] == "") ? null : LocalDate.parse(tokens[5], DateTimeFormatter.ofPattern("yyyy/MM/dd"));
            LocalDate rDate = (tokens[6] == "") ? null : LocalDate.parse(tokens[6], DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            LocalTime rTime = (tokens[7] == "") ? null : LocalTime.parse(tokens[7], DateTimeFormatter.ofPattern("H:mm"));
            LocalDateTime createdAt = (tokens[8] == "") ? null : LocalDateTime.parse(tokens[8], DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss"));

            users[i] = new User(Integer.parseInt(tokens[0]),
                                tokens[1],
                                tokens[2],
                                tokens[3],
                                tokens[4],
