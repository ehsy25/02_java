...
public class AppByTokenizer {

    public static void main(String[] args) {

        String data = "id,first_name,last_name,email,gender,birthday,reservation_date,reservation_time,created_at\n" +
        ...
        int memNum = lines.length - 1;  // 100
        User[] users = new User[memNum];
        StringTokenizer stLine = new StringTokenizer(data, "\n");
        stLine.nextToken();
        int idx = 0;
        while (stLine.hasMoreTokens()) {
            String line = stLine.nextToken();
            StringTokenizer stWordWithD = new StringTokenizer(line, ",", true);
            String[] tokens = new String[stWordWithD.countTokens()];
            int tIdx = 0;
            int twoDlm = 0;
            for  (int i = 0; i < tokens.length; i++) {
                String tmp = stWordWithD.nextToken();
                if ( (tmp.equals(",")) && (twoDlm == 1) ) {
                    tokens[tIdx] = null;
                    twoDlm = 0; tIdx++;
                    continue;
                }
                if ( (tmp.equals(",")) && (twoDlm == 0) ) {
                    twoDlm = 1;
                    continue;
                }
                tokens[tIdx] = tmp;
            ...
            LocalDate birthday = (tokens[5] == null) ? null : LocalDate.parse(tokens[5], DateTimeFormatter.ofPattern("yyyy/MM/dd"));
            LocalDate rDate = (tokens[6] == null) ? null : LocalDate.parse(tokens[6], DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            LocalTime rTime = (tokens[7] == null) ? null : LocalTime.parse(tokens[7], DateTimeFormatter.ofPattern("H:mm"));
            LocalDateTime createdAt = (tokens[8] == null) ? null : LocalDateTime.parse(tokens[8], DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss"));

            users[idx] = new User(Integer.parseInt(tokens[0]),
                                tokens[1],
                                tokens[2],
                                tokens[3],
                                tokens[4],
                                ...
                                rTime,
                                createdAt);
            System.out.println(users[idx].information());
            idx++;
        }
    }
}
