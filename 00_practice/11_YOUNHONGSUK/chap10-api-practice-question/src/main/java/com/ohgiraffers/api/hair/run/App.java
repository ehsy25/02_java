    ...
    public static void main(String[] args) {
        ...
        for(int i=1;i<ua.length;i++){
            ...
            if (fields[5].isEmpty()) {
                ...
                birthday = LocalDate.parse(fields[5], DateTimeFormatter.ofPattern("yyyy/MM/dd"));
            }
            LocalDate reservation_date = LocalDate.parse(fields[6], DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            LocalTime reservation_time = LocalTime.parse(fields[7], DateTimeFormatter.ofPattern("H:mm")); // Handles H:mm or HH:mm
            LocalTime created_at = LocalTime.parse(fields[8], DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss"));

            ua[i-1] = new User(id, first_name,last_name,email,gender,birthday,reservation_date,reservation_time,created_at);
        }

        for (int i = 0; i < ua.length-1; i++) {
            System.out.println(ua[i].toString());
        }
    }
}
