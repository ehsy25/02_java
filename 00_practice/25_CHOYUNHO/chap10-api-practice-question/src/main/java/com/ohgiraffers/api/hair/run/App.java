...
import com.ohgiraffers.api.hair.dto.User;
...
public class App {
    public static void main(String[] args) {
        ...
        User []user = new User[str.length-1];
        ...
        for(int i=1;i<str.length;i++){
            ...
            if(!a[5].isEmpty()){
                try{
                    birthDate = LocalDate.parse(a[5]);
                }catch(Exception e){
                    DateTimeFormatter now = DateTimeFormatter.ofPattern("yyyy/MM/dd");
                    birthDate = LocalDate.parse(a[5],now);
                }
            }else{
                birthDate=null;
            }
            LocalDate reservation_date = LocalDate.parse(a[6]);
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("H:mm");
            LocalTime reservation_time = LocalTime.parse(a[7],formatter);
            LocalDateTime created_at = LocalDateTime.parse(a[8]);
            user[i-1]=new User(id,firstName,lastName,email,gender,birthDate,reservation_date,reservation_time,created_at);
