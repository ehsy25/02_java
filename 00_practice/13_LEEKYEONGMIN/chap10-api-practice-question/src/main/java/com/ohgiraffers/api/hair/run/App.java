...
public class App {
    public static void information(String str){
        ...
        for (int i = 1; i < users.length; i++) {
            ...
            userArr[i] = new User(
                    ...
                    info[3],
                    info[4],
                    LocalDate.parse(info[5], DateTimeFormatter.ofPattern("yyyy/MM/dd")),
                    LocalDate.parse(info[6], DateTimeFormatter.ofPattern("yyyy-MM-dd")),
                    LocalTime.parse(info[7], DateTimeFormatter.ofPattern("H:mm")),
                    LocalDateTime.parse(info[8], DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))
            );
        }*/
        ...
        for (int i = 1; i < users.length-1; i++) {
            ...
            if (!info[5].isBlank()) {
                birthday = LocalDate.parse(info[5], DateTimeFormatter.ofPattern("yyyy/MM/dd"));
            }

            LocalDate reservationDate = null;
            if (!info[6].isBlank()) {
                reservationDate = LocalDate.parse(info[6], DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            }

            LocalTime reservationTime = null;
            if (!info[7].isBlank()) {
                reservationTime = LocalTime.parse(info[7], DateTimeFormatter.ofPattern("H:mm"));
            }

            LocalDateTime createdAt = null;
            if (!info[8].isBlank()) {
                createdAt = LocalDateTime.parse(info[8]);
            }

            userArr[i-1] = new User(
                    Integer.parseInt(info[0]),
                    info[1],
                    info[2],
                    info[3],
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            e.printStackTrace();  // 실제 예외 메시지 전체 출력
        }
