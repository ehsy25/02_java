...
public class Application3 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("==================================");
        // 음수를 마지막 인자로 넣어주게 되면 마지막 구분자 뒤의 값이 존재하지 앟는 경우 빈 문자열로 토큰을 생성
        String[] empArr4 = emp3.split("/", limit:-1);

        System.out.println(Arrays.toString(empArr3));
        System.out.println(Arrays.toString(empArr4));

        String[] empArr5 = emp1.split(regex:"/", limit:3);
        System.out.println(Arrays.toString(empArr5));

        StringTokenizer tokenizer = new StringTokenizer(emp1, "/");
        StringTokenizer tokenizer = new StringTokenizer(emp2, "/");
        StringTokenizer tokenizer = new StringTokenizer(emp3, "/");

        while (st1.hasmoreTokens()) {    // hasMoreTokens(): 현재 꺼낼 토큰이 있는지 없는지 체크
            system.out.println("st1  : " + st1.nextToken());  // nextToken: 다시 꺼낼 토큰
        }
